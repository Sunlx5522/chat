...
public class SignUpController { // 定义一个名为SignUpController的控制器类，处理与注册相关的请求
  ...
  @PostMapping("/signUp")
  // @PostMapping注解用于将HTTP
  // POST请求映射到特定的处理方法上，这里映射到"/signUp"路径，表示当收到POST方式的"/api/signUp"请求时，由signUp方法进行处理。
  public String signUp(@RequestBody Map<String, String> signUpData) { // 定义一个public类型的signUp方法，返回值为String类型。使用@RequestBody注解将请求体中的JSON数据自动转换为Map<String,
                                                                      // String>类型的signUpData参数。
  ...
  private String generateUniqueAccount() { // 定义一个私有方法generateUniqueAccount，用于生成唯一的10位账号
    ...
    return account; // 直接返回生成的账号，不添加"new"前缀
  }
