...
public class LoginController {
    // 定义一个名为LoginController的控制器类，处理与登录相关的请求
    ...
    public String login(@RequestBody Map<String, String> loginData) {
        ...
        // 从loginData中获取键为"password"的值，即用户提交的密码。
        ...
        if (user != null && user.getPassword().equals(password)) {
            // 首先检查user是否不为null，即用户是否存在。
            // 然后比较数据库中存储的密码与用户提交的密码是否匹配。

            return "yes";
            // 如果验证成功，返回登录成功的消息，包含用户的账号信息。

        } else {
            // 如果用户不存在，或者密码不匹配，则执行以下代码。
