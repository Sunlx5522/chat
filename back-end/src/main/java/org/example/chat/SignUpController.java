...
public class SignUpController {  // 定义一个名为SignUpController的控制器类，处理与注册相关的请求
    ...
    @PostMapping("/signUp")  // @PostMapping注解用于将HTTP POST请求映射到特定的处理方法上，这里映射到"/signUp"路径，表示当收到POST方式的"/api/signUp"请求时，由signUp方法进行处理。
    ...
    private String generateUniqueAccount() {  // 定义一个私有方法generateUniqueAccount，用于生成唯一的10位账号
        ...
        String account;  // 声明一个字符串变量account，用于存储生成的账号
        boolean exists;  // 声明一个布尔变量exists，用于检查账号是否已存在

        do {
            account = String.format("%010d", random.nextInt(1000000000));  // 生成10位纯数字账号，不足10位前面补0
            exists = userRepository.findByAccount(account) != null;  // 检查账号是否已经存在于数据库中
        } while (exists);  // 如果账号已存在，重新生成

        return account;  // 返回生成的唯一账号
    }
